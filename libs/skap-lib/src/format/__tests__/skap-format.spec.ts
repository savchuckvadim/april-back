...
// Живые выгрузки НЕ хранятся в git (папка example удалена 2026-08-11):
// фикстурные тесты гоняются только локально, если папку положили рядом.
const describeWithFixtures = fs.existsSync(EXAMPLE_DIR)
    ? describe
    : describe.skip;

describeWithFixtures(
    'SkapFileParseService — реальные фикстуры (август 2024, 61-40762)',
    () => {
