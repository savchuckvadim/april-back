...
/** Резолв смарта «Презентации» для записи элементов crm.item. */
export interface PresentationSmartInfo {
...
interface PresentationInfoCacheEntry {
...
/**
 * Канонический pbx-сервис смарта «Презентации» (связка portal↔bitrix,
 * каркас 1-в-1 с PbxZprSmartService — сознательное зеркало).
 *
 ...
 * которое обновляется установщиком синхронно. Стадии обязательны: как и у
 * ЗПР, stageIdByCode собирается по формуле crm.status динамических типов
 * DT{entityTypeId}_{bxCategoryId}:{suffix}.
 *
 * resolveInfo → null = смарт не установлен (или установлен без стадий) —
 * self-gate потребителей (presentation-flow молча пропускает джобы, отчёт по
 * презентации при этом продолжает жить в сделках «ОП Презентации»).
 */
@Injectable()
export class PbxPresentationSmartService {
    private readonly logger = new Logger(PbxPresentationSmartService.name);
    /**
     ...
     */
    private readonly cacheByDomain = new Map<
        string,
        PresentationInfoCacheEntry
    >();
    ...
    async resolveInfo(domain: string): Promise<PresentationSmartInfo | null> {
        ...
        const fromPortal = await this.resolveFromPortalModel(domain).catch(
            error => {
                this.logger.warn(
                    `PortalModel-резолв pres не удался (${domain}): ${(error as Error).message}`,
                );
                return null;
            },
        );
        const info =
            ...
            (await this.resolveFromDb(domain).catch(error => {
                this.logger.warn(
                    `БД-резолв pres не удался (${domain}): ${(error as Error).message}`,
                );
                return null;
            }));
    ...
    private async resolveFromPortalModel(
        domain: string,
    ): Promise<PresentationSmartInfo | null> {
        const { PortalModel: portalModel } = await this.pbxService.init(domain);
        const smart = portalModel.getSmartByType(PRESENTATION_SMART_TYPE);
        ...
        const enumItems: PresentationSmartInfo['enumItems'] = {};
    ...
    private async resolveFromDb(
        ...
        for (const field of fields) {
            ...
            ufKeyByCode[field.code] =
                field.bitrixCamelId ||
                buildPresentationItemFieldName(typeId, field.code);
            if (field.items?.length) {
                enumItems[field.code] = field.items
        ...
        if (!Object.keys(stageIdByCode).length) {
            ...
            this.logger.warn(
                `pres на ${domain}: строка smarts есть, стадии в зеркале не найдены — resolveInfo=null`,
            );
            return null;
        }
     ...
     * Воронка у презентаций одна; на всякий случай первая категория
     * приоритетна.
     */
    private buildStageIdByCode(
    ...
    ): Partial<Record<PresentationSmartStageCode, string>> {
        const map: Partial<Record<PresentationSmartStageCode, string>> = {};
        for (const category of categories) {
            ...
            for (const stage of category.stages ?? []) {
                ...
                const code = stage.code as PresentationSmartStageCode;
    ...
    async mirrorFields(
        ...
        const camelByNormalized = entityTypeId
            ...
            : {};
        this.logger.log(
            `Зеркало полей pres (${domain}): crm.item.fields дал ` +
                `${Object.keys(camelByNormalized).length} camel-ключей` +
                `${entityTypeId ? '' : ' (entityTypeId не передан — формула)'}`,
        );

        const parentType = `${PRESENTATION_SMART_GROUP}_${PRESENTATION_SMART_TYPE}`;
        ...
        for (const def of defs) {
            ...
            const formulaCamel = buildPresentationItemFieldName(
                typeId,
                def.code,
            );
            const factualCamel =
    ...
    /** UF_CRM_8_PRES_LEAD и ufCrm8PresLead → 'ufcrm8preslead'. */
