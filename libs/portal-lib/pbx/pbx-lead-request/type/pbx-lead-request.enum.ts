...
export enum EnumLeadWorkKindCode {
    ...
    lead = 'lead',
    /**
     * «Неопределён» — робот Битрикса вида НЕ ЗНАЕТ и честно об этом говорит.
     *
     * Это не четвёртый вид работы, а отсутствие ответа: детектор на нём НЕ
     * останавливается, а идёт дальше по признакам (поля лидогена → наши
     * site-метки → `SOURCE_ID`) — ровно как при пустом поле. Поэтому кода
     * нет в `LEAD_WORK_KIND_TO_ITEM_CODE`: сам бэк это значение не пишет
     * никогда — к моменту записи вид работы ему уже известен.
     */
    undef = 'undef',
}
