...
describe('DuplicateScoreService', () => {
    ...
    it('score не выходит за 100', () => {
        const result = service.score([
            hit({ kind: DuplicateSignalKind.INN, via: 'UF_CRM_OP_INN', value: 'a' }),
    ...
    it('одно и то же значение из двух полей считается один раз', () => {
        const twice = service.score([
            hit({ kind: DuplicateSignalKind.INN, via: 'UF_CRM_OP_INN', value: '7707083893' }),
            hit({ kind: DuplicateSignalKind.INN, via: 'UF_CRM_INN_2', value: '7707083893' }),
        ]);
    ...
    it('отсекает слабых кандидатов по порогу и сортирует по убыванию', () => {
        const result = service.score([
            hit({ id: 1, kind: DuplicateSignalKind.TITLE, via: '%TITLE', value: 'ромашка' }),
            hit({ id: 2, kind: DuplicateSignalKind.INN, via: 'UF_CRM_OP_INN', value: '7707083893' }),
        ]);
