...
export const PBX_SALES_EVENT_FIELDS = [
    ...
    /*
     * ДАННЫЕ ЗАЯВКИ НА СДЕЛКЕ (решение владельца 17.09.2026).
     *
     * Менеджер живёт в сделке, а регион, номер заявки, код партнёра и
     * источник лежали только на лиде — за ними приходилось ходить в лид.
     *
     * Лидовые поля УЖЕ СУЩЕСТВУЮТ на портале под этими именами. Парное поле
     * сделки заводим ПОД ТЕМ ЖЕ ИМЕНЕМ намеренно: штатная конвертация
     * Битрикса копирует значение, если поле с одинаковым именем есть у обеих
     * сущностей (проверено 17.09 на сделке 80641). Так их будет переносить и
     * кнопка Битрикса, и наш хук.
     */
    {
        name: 'Регион заявки',
        appType: 'lead',
        type: 'string',
        items: [],
        code: 'lead_user_region',
        lead: 'USER_REGION',
        company: '',
        deal: 'LEAD_USER_REGION',
        smart: '',
        task: '',
        app: 'calling',
        order: 710,
    ...
    },
    {
        ...
        type: 'string',
        items: [],
        code: 'lead_order_number',
        lead: 'ORDER_NUMBER',
        company: '',
        deal: 'LEAD_ORDER_NUMBER',
        smart: '',
        task: '',
        app: 'calling',
        order: 712,
    ...
    {
        name: 'Код партнёра заявки',
        appType: 'lead',
        type: 'string',
        items: [],
        code: 'lead_reg_number',
        lead: 'REG_NUMBER',
        company: '',
        deal: 'LEAD_REG_NUMBER',
        smart: '',
        task: '',
        app: 'calling',
        order: 713,
    ...
    {
        name: 'Рекомендации по работе с клиентом',
        appType: 'lead',
        type: 'string',
        items: [],
        code: 'lead_user_advice',
        lead: 'LEAD_USER_ADVICE',
        company: '',
        deal: 'LEAD_USER_ADVICE',
        smart: '',
        task: '',
        app: 'calling',
        order: 714,
    ...
    },
    {
        ...
        type: 'string',
        items: [],
        code: 'lead_page_referrer',
        lead: 'LEAD_PAGE_REFERRER',
        company: '',
        deal: 'LEAD_PAGE_REFERRER',
        smart: '',
        task: '',
        app: 'calling',
        order: 716,
    ...
    /*
     * Телефоны и почты заявки. У лида это ШТАТНЫЕ поля PHONE/EMAIL, а не UF,
     * поэтому сопоставить по имени нечем — заполняет только наш хук.
     *
     * Звонков в сделке эти поля НЕ дают: телефония привязывает звонок к
     * контакту. Поля показывают номер; звонки появятся после конвертации
     * лида в контакт, привязанный к сделке.
     */
    {
        name: 'Телефоны заявки',
        appType: 'lead',
        type: 'string',
        items: [],
        code: 'op_lead_phones',
    ...
    /*
     * === Аудит сделок (deal-audit) ===================================
     *
     * Поля ТОЛЬКО сделки и ТОЛЬКО на запись краном: это слепок того, что
     * крон посчитал, а не ввод менеджера. Лида и компании здесь нет
     * намеренно — «забытость» меряется по сделке (у неё есть стадия,
     * дата перемещения и задачи), у лида для этого свой SLA-контур.
     *
     * Почему признаков ДВА поля, а не одно. `op_audit_status` — ОДИН
     * худший признак: по нему строят канбан-фильтр «покажи забытые» и
     * условие робота (робот умеет сравнивать одно значение).
     * `op_audit_flags` — ВСЕ сработавшие признаки разом: сделка бывает
     * одновременно и без задачи, и застрявшей, и одного значения для
     * разбора «почему» не хватает. Числа рядом (дни, счётчики) нужны
     * не для красоты, а чтобы фильтр CRM умел «больше N дней» —
     * по enum-признаку так не отфильтруешь.
     */
    {
        name: 'ОП Аудит: статус',
        appType: 'audit',
        type: 'enumeration',
        items: [
            { code: 'op_audit_status_ok', name: 'Норма' },
            { code: 'op_audit_status_no_task', name: 'Без задач' },
            {
                code: 'op_audit_status_task_overdue',
                name: 'Просроченная задача',
            },
            { code: 'op_audit_status_idle', name: 'Давно нет работы' },
            { code: 'op_audit_status_stage_stuck', name: 'Застряла в стадии' },
            {
                code: 'op_audit_status_forgot_close',
                name: 'Забыли закрыть (продажа/отказ)',
            },
        ],
        code: 'op_audit_status',
        lead: '',
        company: '',
        deal: 'OP_AUDIT_STATUS',
        smart: '',
        task: '',
        app: 'calling',
        order: 740,
    ...
    {
        name: 'ОП Аудит: признаки',
        appType: 'audit',
        type: 'enumeration',
        items: [
            { code: 'op_audit_flag_no_task', name: 'Без открытых задач' },
            { code: 'op_audit_flag_task_overdue', name: 'Просрочена задача' },
            {
                code: 'op_audit_flag_task_no_deadline',
                name: 'Задача без дедлайна',
            },
            {
                code: 'op_audit_flag_next_date_empty',
                name: 'Нет даты следующего звонка',
            },
            {
                code: 'op_audit_flag_next_date_past',
                name: 'Дата следующего звонка в прошлом',
            },
            { code: 'op_audit_flag_idle', name: 'Давно нет работы' },
            { code: 'op_audit_flag_stage_stuck', name: 'Застряла в стадии' },
            {
                code: 'op_audit_flag_forgot_close',
                name: 'Забыли закрыть (продажа/отказ)',
            },
            {
                code: 'op_audit_flag_no_responsible',
                name: 'Нет ответственного',
            },
        ],
        code: 'op_audit_flags',
        lead: '',
        ...
        is_rewrite: '',
        isNeedUpdate: true,
        // Множественное: сделка бывает одновременно без задачи и застрявшей.
        isMultiple: true,
    },
    {
        name: 'ОП Аудит: дата проверки',
        appType: 'audit',
        type: 'datetime',
        items: [],
        code: 'op_audit_at',
    ...
    {
        name: 'ОП Аудит: дней без работы',
        appType: 'audit',
        type: 'integer',
        items: [],
        code: 'op_audit_idle_days',
        lead: '',
        company: '',
        deal: 'OP_AUDIT_IDLE_DAYS',
        smart: '',
        task: '',
        app: 'calling',
        order: 743,
    ...
    {
        name: 'ОП Аудит: дней просрочки задачи',
        appType: 'audit',
        type: 'integer',
        items: [],
        code: 'op_audit_overdue_days',
        lead: '',
        company: '',
        deal: 'OP_AUDIT_OVERDUE_DAYS',
        smart: '',
        task: '',
        app: 'calling',
        order: 744,
    ...
    {
        name: 'ОП Аудит: дней в стадии',
        appType: 'audit',
        type: 'integer',
        items: [],
        code: 'op_audit_stage_days',
        lead: '',
        company: '',
        deal: 'OP_AUDIT_STAGE_DAYS',
        smart: '',
        task: '',
        app: 'calling',
        order: 745,
    ...
    {
        name: 'ОП Аудит: открытых задач',
        appType: 'audit',
        type: 'integer',
        items: [],
        code: 'op_audit_open_tasks',
        lead: '',
        company: '',
        deal: 'OP_AUDIT_OPEN_TASKS',
        smart: '',
        task: '',
        app: 'calling',
        order: 746,
    ...
    {
        name: 'ОП Аудит: расшифровка',
        appType: 'audit',
        type: 'string',
        items: [],
        code: 'op_audit_comment',
        lead: '',
        company: '',
        deal: 'OP_AUDIT_COMMENT',
        smart: '',
        task: '',
        app: 'calling',
        order: 747,
