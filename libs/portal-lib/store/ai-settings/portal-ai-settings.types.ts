...
export interface PortalAiSettingsRecord {
    ...
    /**
     * НАЗВАНИЯ НАШИХ ОРГАНИЗАЦИЙ на этом портале: как менеджеры называют
     * СВОЮ компанию клиенту («Альфа-центр», «Апрель», домен почты).
     *
     * Зачем: все клиенты продукта — партнёры Гаранта и представляются
     * по-разному. Разбор, не знавший этих имён, читал их как ЧУЖОЙ бренд
     * и снижал за это оценку (прод alfacentr 08.09.2026). Список уходит в
     * паспорт звонка и в промпты явной формулировкой «это МЫ».
     *
     * NULL или пустой список — прежнее поведение: в промпте не появляется
     * ни строчки, портал без настройки ничего не теряет.
     */
