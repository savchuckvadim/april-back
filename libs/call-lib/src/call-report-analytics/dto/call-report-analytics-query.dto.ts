...
export class CallReportAnalyticsQueryDto {
    ...
    @ApiPropertyOptional({
        description:
            'Bitrix-id менеджера (ответственного сделки). Звонки без ' +
            'сохранённого менеджера (обработанные до включения фичи) при ' +
            'фильтре отбрасываются — их число видно в meta.skippedNoManager.',
        example: '7',
        type: String,
    })
    @IsOptional()
