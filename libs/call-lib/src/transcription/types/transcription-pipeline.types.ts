 ...
 *
 * Жизненный цикл: queued (звонок забронирован сканером и поставлен в
 * очередь) → processing (воркер начал транскрибацию) → done | error.
 *
 * 'queued' появился 23.08.2026: раньше строка создавалась только при
 * СТАРТЕ воркера, поэтому звонок, уже стоящий в очереди, дедупу был
 * невидим — следующий скан ставил его повторно, Bull молча игнорировал
 * дубль по jobId, а слот лимита прохода сгорал впустую.
 */
