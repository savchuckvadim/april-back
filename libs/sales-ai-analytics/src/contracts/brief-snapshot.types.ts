 ...
 *
 * Расход вызова живёт и в нагрузке, и в колонках `ais.tokens_count` /
 * `ais.price` (решение владельца 21.09.2026, вопрос B2): джоба резюме
 * кладёт tokensCount / price ещё и в `usage` конверта (SnapshotUsage), а
 * общий стор приложения (AiAnalyticsSnapshotStore.upsert) пишет их в
 * колонки. Модель провайдера — только здесь, в нагрузке: колонка
 * `ais.model` у всех снапшотов несёт calcVersion. Поля нагрузки остаются
 * ради обратной совместимости читателей и названы как колонки.
 *
