...
describe('toBatchSafeText', () => {
    ...
    it("экранирует '#' — он открывает фрагмент и режет команду до конца", () => {
        // Инцидент 15.09: ссылка `[URL=…]#25639[/URL]` в комментарии
        // таймлайна обрывала команду на решётке — хвост записи пропадал,
        // а незакрытый тег оставался в карточке сырым текстом.
        expect(toBatchSafeText('сделка #25639 закрыта')).toBe(
            'сделка %2325639 закрыта',
        );
    });

    it('комбинация: порядок замен не двоит escape-последовательности', () => {
        // % → %25, + → %2B, & → %26, # → %23, \n → %0A — каждая по разу.
        expect(toBatchSafeText('100% + бонус & тел. +7 #1\nконец')).toBe(
            '100%25 %2B бонус %26 тел. %2B7 %231%0Aконец',
        );
    });
});
