 ...
 *
 * При `domain === 'all'` операция выполняется для всех порталов (компания должна существовать в PortalDB).
 */
@Injectable()
export class PbxCompanyFieldManageUseCase {
    private readonly logger = new Logger(PbxCompanyFieldManageUseCase.name);
    ...
    async deleteFieldItem(
        ...
        for (const domain of domains) {
            ...
            const dbField = await this.findDbField(company.id, dto.fieldCode);
            if (!dbField) {
                results.push({
                    domain,
                    portalId,
