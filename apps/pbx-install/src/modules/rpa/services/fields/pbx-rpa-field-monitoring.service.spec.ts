 ...
 * Bitrix постранично (`userFieldConfig.getAll`), а не одной страницей `list`, иначе
 * поле сверх ~50 на страницу (репро: `UF_RPA_1_SALE_DATE`) пропадает из сводки.
 */
describe('PbxRpaFieldMonitoringService', () => {
    ...
    beforeEach(() => {
        ...
        pbxService = {
            init: jest.fn().mockResolvedValue({
                bitrix: { userFieldConfig: { getAll, list } },
            }),
        };
        portalService = {
    ...
    it('читает Bitrix через getAll(rpa, { entityId }), а не одной страницей list', async () => {
        getAll.mockResolvedValue([]);

        await service.getPbxRpaFieldsByDomain(DOMAIN, RpaNameEnum.SUPPLY);

        expect(getAll).toHaveBeenCalledWith('rpa', { entityId: 'RPA_1' });
    ...
    it('поле, присутствующее и в Bitrix, и в PortalDB, мерджится', async () => {
        ...
        getAll.mockResolvedValue([{ fieldName: 'UF_RPA_1_AMOUNT', id: 1 }]);
