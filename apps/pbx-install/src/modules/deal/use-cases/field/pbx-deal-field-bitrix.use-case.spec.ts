import { PbxDealFieldBitrixUseCase } from './pbx-deal-field-bitrix.use-case';
import {
    BxEntityFieldManageService,
    BxEntityFieldsInstallService,
} from '../../../shared';

describe('PbxDealFieldBitrixUseCase', () => {
    let pbxService: { init: jest.Mock };
    let parseService: { getParsedData: jest.Mock };
    let useCase: PbxDealFieldBitrixUseCase;
    ...
    beforeEach(() => {
        ...
        useCase = new PbxDealFieldBitrixUseCase(
            pbxService as never,
    ...
    it('listFields отдаёт только UF_CRM_-поля', async () => {
        initWith([{ FIELD_NAME: 'UF_CRM_OP_STATUS' }, { FIELD_NAME: 'STAGE' }]);
