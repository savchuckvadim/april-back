...
/**
 * Операции над полями сделки **только в живом Bitrix**, без записи в PortalDB.
 * Источник шаблона — Excel (через ParseEntityFieldsService).
 */
@Injectable()
export class PbxDealFieldBitrixUseCase {
    constructor(
    ...
    private async loadBitrixFields(domain: string): Promise<IBXField[]> {
        ...
        const { result } = await bitrix.deal.getFieldsList();
