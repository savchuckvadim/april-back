...
/**
 * Операции над полями контакта **только в живом Bitrix**, без записи в PortalDB.
 * Источник шаблона — Excel (через ParseEntityFieldsService).
 */
@Injectable()
export class PbxContactBitrixUseCase {
    constructor(
    ...
    private async loadBitrixFields(domain: string): Promise<IBXField[]> {
        ...
        const { result } = await bitrix.contact.getFieldsList();
