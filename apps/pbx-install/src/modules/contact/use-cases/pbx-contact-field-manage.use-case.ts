...
/**
 * Manage-операции над полями контакта Bitrix-портала.
 *
 * 1) deleteFields  — удаляет указанные поля в PortalDB и Bitrix (для enumeration list-items в Bitrix
 *    удаляются автоматически вместе с полем; в PortalDB items удаляются в транзакции вместе с filed).
 * 2) deleteFieldItem — удаляет один элемент enumeration-поля и в PortalDB, и в Bitrix.
 * 3) editFieldItem  — обновляет VALUE/name одного item-а в обоих хранилищах.
 *
 * При `domain === 'all'` операция выполняется для всех порталов (контакт должен существовать в PortalDB).
 */
@Injectable()
export class PbxContactFieldManageUseCase {
    private readonly logger = new Logger(PbxContactFieldManageUseCase.name);
    ...
    async deleteFields(
        ...
        for (const domain of domains) {
            ...
            const contact =
                await this.portalContactService.findByPortalId(portalId);
            if (!contact) {
                this.logger.warn(`contact not found for portalId ${portalId}`);
                continue;
            }
    ...
    async deleteFieldItem(
        ...
        for (const domain of domains) {
            ...
            const dbField = await this.findDbField(contact.id, dto.fieldCode);
            if (!dbField) {
                results.push({
                    domain,
                    portalId,
