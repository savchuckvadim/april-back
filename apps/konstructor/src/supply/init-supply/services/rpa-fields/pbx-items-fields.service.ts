...
import { EntityFormFieldDto } from '@app/konstructor/document-generate/dto/entity-form-field/entity-form-field.dto';
...
export class InitSupplyRpaPbxItemsFieldsService {
    public async get(dto: InitSupplyDto, PortalModel: PortalModel) {
        ...
        const bxCompanyItemsRpaValues = this.getBxCompnyItemsRpaValues(
            dto.bxCompanyItems,
            PortalModel,
        );
        const bxDealItemsRpaValues = this.getBxDealItemsRpaValues(
            dto.bxDealItems,
            PortalModel,
        );
    ...
    private getBxCompnyItemsRpaValues(
        ...
        for (const key in bxCompanyItems) {
            ...
            if (pbxItem.current) {
                const fieldResult = this.processPbxEntityField(
                    pbxItem,
                    PortalModel,
                    key,
                );
    ...
    private getBxDealItemsRpaValues(
        ...
        for (const key in bxDealItems) {
            ...
            const isTargetField = enumValues.includes(
                pbxItem.field.code as PbxDealEnum,
            );
    ...
    private processPbxEntityField(
        pbxItem: EntityFormFieldDto,
        PortalModel: PortalModel,
        key: string,
        ...
        if (pbxItem.current) {
            if (
                pbxItem.field.code === 'op_source_select' ||
                pbxItem.field.code === 'situation_comments'
            ) {
                //пропускаем источник
                return undefined;
            }
            if (typeof pbxItem.current === 'string') {
                ...
                const rpaFieldItemBitrixId = PortalModel.getFieldItemByCode(
                    rpaField,
                    pbxItem.current.code,
                );
