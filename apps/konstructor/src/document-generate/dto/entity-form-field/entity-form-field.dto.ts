import { PbxEntityTypePrisma } from '@lib/shared/enums';
import {
    IsNotEmpty,
    IsObject,
    IsOptional,
...
export class PbxFieldDto {
    ...
    @ValidateNested({
        each: true,
        message: 'each item must be a valid PbxFieldItemDto',
    })
...
export class EntityFormFieldDto {
    ...
    @IsObjectOrStringOrNull({
        message: 'current must be a valid PbxFieldItemDto or string or null',
    })
