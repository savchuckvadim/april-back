...
import { IsObjectOrStringOrNull } from '@lib/core/decorators/dto/object-or-string.decorator';
...
export class FormFieldDto {
    ...
    @IsEnum(FormFieldTypeEnum, {
        message: 'FormFieldDto type must be a valid type',
    })
    ...
    @IsEnum(RQ_TYPE, {
        each: true,
        message: 'FormFieldDto includes each item must be a valid RQ_TYPE',
    })
    ...
    @IsEnum(SupplyTypeEnum, {
        each: true,
        message:
            'FormFieldDto supplies each item must be a valid SupplyTypeEnum',
    })
    ...
    @IsEnum(CONTRACT_LTYPE, {
        each: true,
        message:
            'FormFieldDto contractType each item must be a valid CONTRACT_LTYPE',
    })
    ...
    @IsEnum(CONTRACT_RQ_GROUP, {
        message: 'FormFieldDto group must be a valid CONTRACT_RQ_GROUP',
    })
    ...
    @IsString({
        message:
            'FormFieldDto component must be a valid component "base" | "contract" | "invoice" | "client" ',
    })
