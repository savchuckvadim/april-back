...
import { OrkFields } from '@lib/portal-lib/pbx/pbx-ork-history-bx-list';
...
export class KpiReportOrkEventService {
    ...
    async getReport(domain: string) {
        ...
        const pbxList = portal.getListByCode('service_ork_history');
        if (!pbxList) {
        ...
        Object.values(OrkFields).forEach(field => {
            if (field.code === OrkFields.ork_event_action.code) {
                Object.values(OrkFields.ork_event_action.items).forEach(
                    item => {
