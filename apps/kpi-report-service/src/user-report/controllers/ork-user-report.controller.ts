...
import { OrkListHistoryItemDto } from '@lib/portal-lib/pbx/pbx-ork-history-bx-list/dto/ork-list-history.dto';
...
export class UserReportController {
    ...
    @ApiResponse({
        status: 200,
        description: 'Report',
        type: OrkUserReportStartResponseDto,
    })
    @Post()
    async start(@Body() body: OrkUserReportGetRequestDto) {
        ...
        await redis.expire(
            'ork-user-report-operation:' + job.id.toString(),
            3600,
        ); // 1 hour TTL
        return new OrkUserReportStartResponseDto(
            job.id.toString() || '0',
    ...
    ): Promise<OrkUserReportStartResponseDto> {
        const redis = this.redis.getClient();
        const hash = await redis.get(
            'ork-user-report-operation:' + operationId,
        );
