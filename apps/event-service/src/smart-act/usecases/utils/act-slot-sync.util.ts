...
/**
 * Целевая стадия слота месяца договора:
 * - месяц полностью прошёл (monthIndex <= passedMonths) → success («Сдан»);
 * - текущий незавершённый месяц (monthIndex === passedMonths + 1) → inwork («Выписан», в работе);
 * - будущий месяц (monthIndex > passedMonths + 1) → planned («Запланирован»).
 *
 * Важно: при now >= end договора passedMonths === totalMonths, все слоты доходят до success
 * (в т.ч. последний) — ни один не остаётся inwork.
 */
export function resolveCreateStageForSlot(
    monthIndex: number,
    passedMonths: number,
): 'success' | 'inwork' | 'planned' {
    if (monthIndex <= passedMonths) {
        return 'success';
    }
    if (monthIndex === passedMonths + 1) {
        return 'inwork';
    }
    return 'planned';
}
