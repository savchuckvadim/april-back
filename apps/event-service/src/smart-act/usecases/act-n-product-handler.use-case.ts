...
export class ActNProductHandlerUseCase {
    ...
    async execute(
        ...
        let { plans } = await this.orkActsReconcilePlanUseCase.execute(
            domain,
            dealId,
        );
        ...
        if (didShiftContractStart) {
            ...
            ({ plans } = await this.orkActsReconcilePlanUseCase.execute(
                domain,
                Number(dealId),
            ));
        }
        for (const plan of plans) {
            ...
            await this.syncDealActsFromPlan(domain, plan);
        }
        // 8) Задачи-предупреждения: сдвиг «начала договора» (changed_from) + проблемы по плану (один batch).
        //    При withTasks=false (напр. часть прогонов по расписанию) акты синхронизируем, но задачи не ставим.
        if (withTasks) {
            const planWarningTasks = plans.flatMap(p =>
                buildSmartActWarningTasksFromPlan(p),
            );
            const warningTasks = [
                ...changedFromContractTasks,
                ...planWarningTasks,
            ];
            if (warningTasks.length > 0) {
                const warningTaskService = new SmartActWarningTaskService(
                    bitrix,
                );
    ...
    /**
     * Сопоставляет слоты expectedActPeriods (на ВЕСЬ срок договора) с актами по UF-периоду,
     * создаёт недостающие: прошедшие месяцы → success (Сдан), текущий → inwork (Выписан), будущие → planned (Запланирован),
     * переводит стадию существующих актов вперёд по процессу до целевой (без отката назад),
    ...
    private async syncDealActsFromPlan(
        ...
        for (let monthIndex = 1; monthIndex <= slotsCount; monthIndex++) {
            ...
            // Нет акта под этот месяц — создаём (прошедшие — success/Сдан, текущий — inwork/Выписан, будущие — planned/Запланирован).
            if (!match) {
            ...
            // Перевод стадии под целевую для слота, но только вперёд по процессу
            // (прошедший месяц → Сдан, текущий → Выписан, будущий → Запланирован);
            // назад не откатываем и ручные «У клиента»/«Оплачен» не трогаем (см. ensureStageForward).
            await smartActService.ensureStageForward(
                actId,
                match.stageId,
                targetStage,
            );
