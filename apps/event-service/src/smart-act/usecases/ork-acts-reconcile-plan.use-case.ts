...
export class OrkActsReconcilePlanUseCase {
    ...
    private buildDealPlan(item: IDealWithRows): IDealReconcilePlan {
        ...
        // Акты создаются сразу на весь срок договора (прошедшие → Сдан, текущий → Выписан, будущие → Запланирован),
        // поэтому ожидаемое число актов = месяцам договора, а не только до текущего месяца.
        const expectedActsTotal = totalMonths; //ожидаемые акты (на весь договор)
    ...
    private getInprogressActQuantity(
        ...
        const inprogress = smartItems.items.filter(
            item =>
                item.stageType === 'planned' ||
                item.stageType === 'inwork' ||
                item.stageType === 'new',
        );
