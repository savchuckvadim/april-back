...
export class CategorySmartActService {
    ...
    /** «Выписан» (CLIENT) — стадия «в работе» для текущего месяца договора. */
    public isInWorkStage(item: IBXItem): boolean {
        return item.stageId?.includes('CLIENT') || false;
    }
    public async getSmartStageDataForCreate(
        ...
        new: IGetStageData;
        planned: IGetStageData;
        inwork: IGetStageData;
        success: IGetStageData;
    }> {
        return {
            new: await this.getNewStageData(domain, targetSmart),
            planned: await this.getPlannedStageData(domain, targetSmart),
            inwork: await this.getInWorkStageData(domain, targetSmart),
    ...
    /** «Запланирован» (PREPARATION) — стадия для будущих месяцев договора. */
    private async getPlannedStageData(
        domain: string,
        targetSmart: IPSmart,
    ): Promise<IGetStageData> {
        const stages = await this.getCategorySmartAct(domain, targetSmart);
        ...
        const plannedStage =
            stages.stages.find(
                stage =>
                    stage.STATUS_ID?.includes('PREPARATION') ||
                    stage.SYSTEM === 'N',
            ) || null;
        if (!plannedStage) {
            return {
    ...
    /** «Выписан» (CLIENT) — стадия «в работе» для текущего незавершённого месяца. */
    private async getInWorkStageData(
        domain: string,
        targetSmart: IPSmart,
    ): Promise<IGetStageData> {
        const stages = await this.getCategorySmartAct(domain, targetSmart);
        ...
        const inWorkStage =
            stages.stages.find(stage => stage.STATUS_ID?.includes('CLIENT')) ||
            null;
        if (!inWorkStage) {
            return {
                stage: null,
