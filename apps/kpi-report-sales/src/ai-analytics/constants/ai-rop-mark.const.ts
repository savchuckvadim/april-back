...
export const AI_ROP_MARK_MESSAGES = {
    ...
    /**
     * Суперпользователь вендора смотрит слепую проверку только на чтение:
     * метки и пересборку подбора делают руководители портала, иначе вендор
     * засоряет калибровку РОПов.
     */
    superUserReadOnly:
        'Суперпользователь вендора смотрит слепую проверку только на чтение: ' +
        'ставить метки и пересобирать подбор недели могут только руководители ' +
        'портала',
} as const;
