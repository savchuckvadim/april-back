...
/** Пороги счётчиков (документ §2.1); дефолты — коды реестра `style_*`. */
export interface StyleCrmThresholds {
    /** Разговор — не короче стольких секунд при коде 200 (`style_conversation_min_sec`). */
    conversationMinSec: number;
    /** Звонок дня учитывается в темпе от стольких секунд (`style_tempo_min_sec`). */
    tempoMinSec: number;
    /** «Вторая попытка» ожидается в столько рабочих дней (`style_give_up_workdays`). */
    giveUpWorkdays: number;
    /** Обещание выполнено при звонке в ±столько дней (`style_promise_window_days`). */
