 ...
 * контейнер живёт в UTC, тик крона — ежечасный на минуте слота
 * (constants/ai-cron.const.ts), а «пора ли» решается по часам портала.
 ...
 * раньше. Запоздавший на минуту тик слот не теряет. Слот, выпавший на
 * «пропущенный» час перевода на летнее время, в этот день не ставится (у
 * наших слотов 03:15+ и европейского перевода 02:00→03:00 такого нет).
 */
import { DEFAULT_WORK_CALENDAR } from '@lib/sales-ai-analytics';
import { AiLocalSlot } from '../constants/ai-cron.const';
