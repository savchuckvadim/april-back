...
describe('Запись «Презентация состоялась» в pres_comments', () => {
    ...
    /*
     * Поля сущностей уезжают batch-командой: сырой перенос строки доехал бы
     * до карточки подчёркиванием. И разделитель, и сам (многострочный по
     * построению) хвост обязаны быть экранированы.
     */
    it('переносы строк экранированы для batch (%0A), сырых нет', () => {
        const ctx = makeCtx({
            init: {
                lead: {
                    ID: '42',
        ...
        expect(line).toContain('%0AХвост: дожать:%0A- КП%0A- договор');
