 ...
 *
 * Четыре режима:
 *  - `none` — задачи не трогаются вообще (и новая НЕ создаётся: конвертации
 *    автозадача не нужна);
 *  - `move` — открытые задачи переезжают на нового ответственного с
 *    идемпотентным префиксом «Звонок» (повторный хук не даёт «Звонок
 *    Звонок…») и union CRM-привязок; открытых задач не было вовсе — ставится
 *    новая, иначе клиент остался бы без следующего шага;
 *  - `move_keep` — то же, но БЕЗ новой задачи: массовый перенос исторической
 *    базы (15.09.2026) переносит состояние, а не назначает работу;
 *  - `close` (а также любая ХО-ветка) — открытые задачи закрываются и
 *    ставится одна новая.
 */
export class TaskFlowService extends LeadToWorkFlowBase {
    queue(
        ...
        /*
         * Новая задача: при close-режиме всегда; при move — если открытых
         * задач не было (клиент не должен остаться без следующего шага).
         *
         * `move_keep` этот хвост ОТКЛЮЧАЕТ — единственное его отличие от
         * `move`. Нужен массовому переносу исторической базы: там половина
         * лидов без открытых задач, и обычный `move` вывалил бы менеджерам
         * тысячи задач «Звонок» за один прогон.
         */
        const needNewTask =
            closeAll ||
            (ctx.openTasks.length === 0 && item.taskMode !== 'move_keep');
        ...
        const payload: BxRow = {
            ...
            RESPONSIBLE_ID: item.responsible,
            /*
             * ПОСТАНОВЩИК = ОТВЕТСТВЕННЫЙ.
             *
             * Без явного CREATED_BY Битрикс ставит постановщиком владельца
             * интеграции — у нас это админ портала, и каждая созданная хуком
             * задача падала ему в «Я поставил». Проверено 16.09.2026: у
             * задач, созданных кодом, постановщик = 1, у созданных людьми —
             * реальный автор.
             *
             * ЗАДЕЛ НА БУДУЩЕЕ: здесь появятся режимы из админки — «сам себе»
             * (сейчас), «из поля xo_created» и «из boss_id». Поле
             * «ОП Постановщик ХО» для этого уже есть, но брать постановщика
             * из него СЕЙЧАС нельзя: робот портала кладёт туда руководителя,
             * и задачи снова пошли бы от него.
             */
