 ...
 *
 * Поле — источник истины о виде работы: его пишут только наш хук и робот
 * Битрикса (перед постановкой в очередь ХО), поэтому оно переживает и
 * ручные правки карточки, и разницу настроек порталов (чего нельзя сказать
 * о штатном `SOURCE_ID`).
 *
 * Четвёртого значения поля — `undef` («Неопределён») — здесь НЕТ намеренно:
 * это не вид работы, а отсутствие ответа робота. Бэк его не пишет никогда
 * (к моменту записи вид известен), а на чтении `leadWorkKindByItemCode`
 * отдаёт по нему `null`, и детектор идёт дальше по признакам.
 */
