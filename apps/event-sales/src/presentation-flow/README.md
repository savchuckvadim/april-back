...
3. **Спонтанная** (`isSpontaneous` или открытого элемента нет): элемент
   ...
   **Исход спонтанной всегда `done`** и НЕ выводится из флагов отчёта
   (`buildPresentationFlowJobs`). `isUnplannedPresentation` уже означает
   «презентация состоялась» (гейт — `isPresentationDone`), а сам отчёт
   пришёл по ЧУЖОЙ задаче: её `resultStatus` бывает `NEW`/`NORESULT` или
   отсутствует вовсе, и общий вывод давал `noresult` — элемент рождался
   сразу в «Не состоялась». Это зеркало ветки 3
   `SalesPresentationDealService`, где unplanned pres-сделка создаётся с
   `eventAction: done` жёстко, без участия `isResult`. Отказ, приехавший
   тем же отчётом, живёт на базовой сделке — элемент презентации
   фиксирует ФАКТ проведения, а не судьбу клиента.

   В ЗПР этой развилки нет и быть не может: `buildZprFlowJobs` заводит
   джоб только когда `reportEventType === 'hot'`, то есть отчёт и есть про
   этот звонок — там `isResult` описывает именно его, и «не дозвонились →
   Не состоялся» верно.
...
- **Снимок «5К»/«Хвост»** (`job.survey`) — состояние клиента на момент
  ...
  Источники снимка по приоритету: **payload отчёта**
  (`presentation.survey` — ответы приехали вместе с отчётом) → лид →
  базовая сделка. Чтение сущностей осталось ради ЛЕГАСИ-пути (старый фронт
  шлёт анкету отдельным запросом в ручку `/presentation-survey`); на новом
  пути снимок собирается, даже когда лид и сделка пусты — «анкету
  отправили после отчёта» и «во встройке в сделку лида нет» перестали быть
  ловушками.
