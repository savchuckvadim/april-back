...
export interface SurveySummaryValues {
    ...
    /** «Разговор» (op_talk_*): тот же контракт и та же деградация. */
 ...
 *
 * ЛЕГАСИ-МОСТ для старого React-фронта, и существует он ровно потому, что
 * тот фронт шлёт анкету ОТДЕЛЬНЫМ запросом: спонтанную сделку презентации
 * создаёт hook, порядок прибытия непредсказуем, и стороны приходится
 * женить через Redis. Новому фронту это не нужно: ответы едут в payload
 * отчёта (`presentation.survey`), а спонтанную сделку создаёт САМ поток
 * (`set_unplanned_pres_deal`), уже держа ответы в руках. Удалить вместе со
 * старым фронтом.
 *
