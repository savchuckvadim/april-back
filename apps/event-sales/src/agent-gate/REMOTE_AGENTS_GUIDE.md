...
## 1. Ключи доступа (защита эндпоинтов)
...
2. Пропишите в env event-sales (`apps/event-sales/.env` или прод-секреты):
   ...
   ```
   AGENT_API_KEYS=claw-main:9f2c...64hex,claw-experimental:aa10...64hex
   ```

   Формат: `имя:ключ` через запятую. **Имя** попадает в `ais.provider` и в
   поле смарта «Имя агента-аналитика» — по нему потом видно, какой агент
   какой анализ сделал. Отзыв ключа = убрать запись из env и перезапустить
   event-sales.
