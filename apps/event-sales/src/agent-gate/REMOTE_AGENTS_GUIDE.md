...
## 8. Классификация типа звонка и профили типов (обновление 2026-07-23)

Конвейер теперь сам классифицирует звонок дешёвой моделью (VibeCode)
сразу после транскрибации. Для агента это означает:

1. **`GET /agent/calls`** — у каждого pending-звонка есть `callType`
   (cold/call/presentation/decision/payment/other), список отсортирован
   по (domain, callType). Обрабатывайте подряд: методология одного типа
   переиспользуется из кэша промпта.
2. **`GET /agent/calls/:id`** — в пакете добавлено:
   - `classification` — полный результат классификатора (callType,
     interlocutorRole, confidence, reason). При высокой confidence
     принимайте тип; при низкой или несогласии — классифицируйте сами
     и объясните расхождение (бэк логирует mismatch в
     `ais.user_result.classifierMismatch`).
   - `typeProfile` / `typeProfiles` — профиль типа: приоры релевантности
     разделов (sectionRelevance), речевые нормы (talkRatioNorm,
     questionsNorm) и `knowledgeKind` — kind базы знаний с инструкцией
     анализа ИМЕННО этого типа. Выбрали другой тип — работайте по
     профилю выбранного (`typeProfiles[<ваш тип>]`).
3. **Знания по типам** — соглашение kind: `call-analysis-<тип>`
   (например `call-analysis-cold`). Забирайте
   `GET /agent/knowledge/all?kind=call-analysis-cold` вместо всей базы —
   меньше токенов, точнее анализ.
4. **Инструкция классификатора подменяема без деплоя** — документ kind
   `call-classify` (общий или домена) полностью замещает встроенный
   промпт классификатора конвейера. Коды ответа фиксированы схемой.
5. **Речевые метрики считает бэк** — `talkRatioPct` и `questionsCount`
   пересчитываются кодом из вашего `dialog[]` и перекрывают присланные
   значения. Присылайте качественную разметку ролей — числа выведутся.

---

## Смена состава чек-листа «Хвост / 5К» (01.09.2026) — АГЕНТА НАДО ОБНОВИТЬ

Состав анкеты менеджера переделан, и вместе с ним сменились ключи полей
`hvostSteps` и `fiveKItems` в теле `POST /api/agent/calls/{id}/analysis`.

**Почему это не может «просто работать».** Валидация приложения настроена как
`whitelist: true, forbidNonWhitelisted: false`: неизвестные ключи тела
**вырезаются молча** — ни ошибки, ни 400. Агент на старом наборе получит 200,
а чек-лист приедет пустым и в карточке будет выглядеть как «AI не смог
разобрать звонок». С 01.09 такой случай пишется в лог предупреждением
(`warnStaleChecklistShape`): ищите строку «прислал разбор, но hvostSteps
пуст».

### Было → стало

`hvostSteps` (пять смысловых блоков вместо трёх галочек и двух дат):

| было | стало | вопрос менеджеру |
|---|---|---|
| `offer` | `desire` | ЖЕЛАНИЕ РАБОТАТЬ С ГАРАНТОМ: впечатление, что запомнилось, с чем хотел бы работать |
| `complect` | `offered` | ЧТО ПРЕДЛОЖИЛИ: кому нужна СПС, какое наполнение и цену озвучили |
| `price` | `priceReaction` | РЕАКЦИЯ НА ЦЕНУ: как клиент отреагировал |
| `decisionDate` | `decisionProcess` | ПРОЦЕСС ПРИНЯТИЯ РЕШЕНИЯ: кто и как решает, контакты коллег |
| `dateAgreed` | `decisionWay` | ВЫХОД НА РЕШЕНИЕ: готовность продвигать, документы, о чём договорились |

Соответствие в таблице — по ПОЗИЦИИ, а не по смыслу: вопросы разные, старые
ответы в новые пункты не переводятся.

`fiveKItems` (пять блоков вместо девяти вопросов):

| было (9) | стало (5) |
|---|---|
| `clientWhat`, `clientReady`, `clientPrice` | `client` |
| `companyWho`, `companyHow`, `companyRight` | `company` |
| `colleagues` | `colleagues` |
| `competitor` | `competitor` |
| `criteria` | `criteria` |

### Что поменять в скилле агента

1. Ключи ответа — по таблицам выше.
2. Формулировки вопросов — они теперь собираются в промпте из анкеты
   менеджера автоматически (`HVOST_CHECKLIST_SPEC` / `FIVE_K_CHECKLIST_SPEC`
   в `call-deep-analysis.contract.ts`). Актуальный текст всегда там; в скилл
   его дублировать не нужно, достаточно правильных ключей.
3. `hvostDone` — «пройден» теперь означает все ПЯТЬ блоков «✓», а не четыре.
4. `hvostAnalysis` / `fiveKAnalysis` — разбор по пяти блокам в том же
   порядке, что в анкете.

### Проверка после обновления

Прогнать один звонок типа `presentation` и убедиться:

- в элементе смарта «AI-анализ звонков» заполнены `HVOST_DESIRE`,
  `HVOST_OFFERED`, `HVOST_PRICE_REACTION`, `HVOST_DECISION_PROCESS`,
  `HVOST_DECISION_WAY`, `FIVE_K_CLIENT`, `FIVE_K_COMPANY`,
  `FIVE_K_COLLEAGUES`, `FIVE_K_COMPETITOR`, `FIVE_K_CRITERIA`;
- `HVOST_DONE` / `FIVE_K_DONE` пересчитаны из них;
- в логе приложения НЕТ предупреждения «прислал разбор, но hvostSteps пуст».
