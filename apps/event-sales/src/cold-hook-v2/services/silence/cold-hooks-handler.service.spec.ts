...
const CATEGORIES = [
    ...
    },
    {
        ...
        code: 'sales_xo',
        title: 'ХО',
        stages: [stage('cold_plan', 'PLAN'), stage('cold_noresult', 'NORESULT')],
    },
];
...
const makeBitrix = (world: World) => {
    ...
    const bitrix = {
        ...
        deal: {
            all: jest.fn(async (filter: Row) => world.deals.filter(d => matches(d, filter))),
        },
        item: {
        ...
        batch: {
            deal: {
                get: rec('deal.get', id => world.deals.find(d => d.ID === String(id)) ?? null),
                getList: rec('deal.getList', filter => world.deals.filter(d => matches(d, filter as Row))),
                update: rec('deal.update', () => ({})),
                set: rec('deal.set', () => 999),
            },
            company: {
                get: rec('company.get', id => world.companies.find(c => c.ID === String(id)) ?? null),
                update: rec('company.update', () => ({})),
            },
            task: {
                getList: rec('task.getList', filter => ({
                    tasks: world.tasks.filter(t =>
                        t.ufCrmTask.includes(String((filter as Row).UF_CRM_TASK?.[0])),
                    ),
                })),
...
const makeHandler = (world: World) => {
    ...
    const pbx = {
        init: jest.fn(async () => ({ bitrix, portal: { domain: 'd.b24.ru' }, PortalModel })),
    } as unknown as PBXService;
    const presSmart = { resolveInfo: jest.fn(async () => PRES_INFO) } as unknown as PbxPresentationSmartService;
    const zprSmart = { resolveInfo: jest.fn(async () => ZPR_INFO) } as unknown as PbxZprSmartService;
    const names = {
        resolve: jest.fn(async () => ({ 447: 'Вадим Савчук', 448: 'Иван Петров' })),
    } as unknown as UserNameResolver;
    const handler = new ColdHooksHandlerV2Service(pbx, presSmart, zprSmart, names);
...
describe('ColdHooksHandlerV2Service — компания, force=Y, клиент у другого сотрудника', () => {
    const world: World = {
        ...
        deals: [
            { ID: '500', CATEGORY_ID: '17', STAGE_ID: 'C17:WARM', COMPANY_ID: '7', ASSIGNED_BY_ID: '448' },
            { ID: '510', CATEGORY_ID: '48', STAGE_ID: 'C48:PLAN', COMPANY_ID: '7', ASSIGNED_BY_ID: '448', UF_CRM_TO_BASE_SALES: '500' },
            { ID: '490', CATEGORY_ID: '17', STAGE_ID: 'C17:WON', COMPANY_ID: '7', ASSIGNED_BY_ID: '448' },
        ],
        tasks: [{ id: '31', ufCrmTask: ['CO_7', 'D_510'] }],
        items: {
            '1040': [{ id: 11, stageId: 'DT1040_9:PLAN', ufCrm7BaseDeal: 500 }],
            '1038': [{ id: 21, stageId: 'DT1038_9:PLAN', ufCrm8Company: 'CO_7' }],
        },
    };
    ...
    it('закрывает чужую работу, кроме свежей основной, и создаёт холодную на responsible', async () => {
        ...
        expect(of(journal, 'deal.update').map(([, key]) => key)).toEqual(
            expect.arrayContaining(['xo2_close_deal_h1_510', 'update_base_deal_500']),
        );
        expect(journal).toContainEqual([
            'deal.update',
            'xo2_close_deal_h1_510',
            510,
            { STAGE_ID: 'C48:NORESULT', UF_CRM_CALL_NEXT_DATE: '', UF_CRM_XO_DATE: '' },
        ]);
        expect(of(journal, 'task.complete').map(([, , id]) => id)).toEqual([31]);
        ...
        expect(of(journal, 'company.update')[0][1]).toBe('xo_hook_update_event_entity_company_7');
        expect(journal).toContainEqual([
            'deal.update',
            'update_base_deal_500',
            500,
            expect.objectContaining({ ASSIGNED_BY_ID: '447', COMPANY_ID: '7', STAGE_ID: 'C17:PREPARATION' }),
        ]);
        expect(journal).toContainEqual([
            'deal.set',
            'new_cold_deal_7',
            expect.objectContaining({ COMPANY_ID: '7', UF_CRM_TO_BASE_SALES: '500' }),
        ]);
    ...
    it('таймлайн: итог во входные сущности, «забрали» — владельцу основной; push ему же', async () => {
        ...
        const timeline = of(journal, 'timeline.add').map(([, key, data]) => [key, (data as Row).COMMENT]);
        ...
        expect(String(timeline[0][1])).toContain('[B]Холодный старт[/B] — ответственный: Вадим Савчук.');
        expect(String(timeline[2][1])).toContain('[B]Вашу компанию забрали в работу[/B]: Вадим Савчук');
        expect(bitrix.imNotify.systemAdd).toHaveBeenCalledWith(
            expect.objectContaining({
                USER_ID: 448,
                TAG: 'xo2_cold_start_co_7_448',
                MESSAGE: expect.stringContaining('[B]У вас забрали компанию в работу[/B]'),
            }),
        );
    });
});
...
describe('ColdHooksHandlerV2Service — сделка без компании', () => {
    const foreignWorld: World = {
        companies: [],
        deals: [
            { ID: '600', CATEGORY_ID: '48', STAGE_ID: 'C48:PLAN', COMPANY_ID: '', ASSIGNED_BY_ID: '447', LEAD_ID: '12', UF_CRM_TO_BASE_SALES: 'D_77' },
            { ID: '77', CATEGORY_ID: '17', STAGE_ID: 'C17:WARM', COMPANY_ID: '', ASSIGNED_BY_ID: '448', LEAD_ID: '12' },
        ],
        tasks: [
        ...
        items: {
            ...
            '1038': [{ id: 21, stageId: 'DT1038_9:PLAN', ufCrm8BaseDeal: 'D_77' }],
        },
    };
    ...
    it('force=N при чужой основной: уступаем — закрыта только входная и её привязки, ничего не создано', async () => {
        ...
        expect(of(journal, 'deal.update').map(([, key]) => key)).toEqual(['xo2_close_deal_h2_600']);
        expect(of(journal, 'task.complete').map(([, , id]) => id)).toEqual([41]);
        expect(of(journal, 'item.update').map(([, key]) => key)).toEqual(['xo2_close_pres_h2_11']);
        ...
        const timeline = of(journal, 'timeline.add').map(([, key, data]) => [key, (data as Row).COMMENT]);
        expect(timeline.map(([key]) => key)).toEqual(['xo2_tl_h2_0_deal_600', 'xo2_tl_h2_1_deal_77']);
        expect(String(timeline[0][1])).toContain('[B]Холодный старт уступлен[/B]: клиент в работе у Иван Петров');
        expect(String(timeline[1][1])).toContain('[B]Попытка взять вашего клиента в работу[/B]: Вадим Савчук');
        expect(bitrix.imNotify.systemAdd).toHaveBeenCalledWith(
            expect.objectContaining({
                USER_ID: 448,
                MESSAGE: expect.stringContaining('[B]У вас попытались забрать клиента в работу[/B]'),
            }),
        );
    ...
    it('force=N без чужой работы: полный старт без COMPANY_ID, с лидом и контактом входной', async () => {
        const world: World = {
            companies: [],
            deals: [
                { ID: '600', CATEGORY_ID: '17', STAGE_ID: 'C17:WARM', COMPANY_ID: '', ASSIGNED_BY_ID: '447', LEAD_ID: '12', CONTACT_ID: '9' },
            ],
            tasks: [],
            items: {},
        };
        ...
        expect(of(journal, 'deal.update').map(([, key]) => key)).toEqual(['update_base_deal_600']);
        ...
        expect(journal).toContainEqual([
            ...
            expect.objectContaining({
                UF_CRM_TASK: ['D_600', 'D_$result[new_cold_deal_deal_600]', 'L_12'],
            }),
        ]);
        expect(of(journal, 'company.update')).toHaveLength(0);
        expect(bitrix.imNotify.systemAdd).not.toHaveBeenCalled();
        expect(of(journal, 'timeline.add').map(([, key]) => key)).toEqual(['xo2_tl_h3_0_deal_600']);
    });
});
...
describe('ColdHooksHandlerV2Service — два хука в одном окне', () => {
    it('чтение и закрытие обоих идут ДО записи; группы создания уезжают одним flush', async () => {
        const world: World = {
            ...
            deals: [
                { ID: '500', CATEGORY_ID: '17', STAGE_ID: 'C17:WARM', COMPANY_ID: '7', ASSIGNED_BY_ID: '447' },
                { ID: '800', CATEGORY_ID: '17', STAGE_ID: 'C17:WARM', COMPANY_ID: '8', ASSIGNED_BY_ID: '447' },
            ],
            tasks: [],
            items: {},
        };
        ...
        const READS = ['deal.get', 'company.get', 'deal.getList', 'task.getList'];
        const CREATES = ['company.update', 'deal.set', 'task.add', 'listItem.add', 'timeline.add'];
        const readIdx = names.map((n, i) => (READS.includes(n) ? i : -1)).filter(i => i >= 0);
        const createIdx = names.map((n, i) => (CREATES.includes(n) ? i : -1)).filter(i => i >= 0);
        ...
        expect(of(journal, 'deal.set').map(([, key]) => key)).toEqual(['new_cold_deal_7', 'new_cold_deal_8']);
        expect(of(journal, 'task.add').map(([, key]) => key)).toEqual(['bx_task_add_7', 'bx_task_add_8']);
        // Между фазой чтения и итоговым flush чужие батчи группы не уносят:
        // после последней записи создания ровно один вызов батча.
        const calls = (bitrix.api.callBatchWithConcurrency as jest.Mock).mock.calls.length;
