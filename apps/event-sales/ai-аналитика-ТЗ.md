...
**FR-41 Горячий клиент и пайплайн от стадии.** «Горячий» в UI — только событие `hot`; открытые сделки `sales_base` со стадией ≥ порога — «пайплайн от стадии X»: `count`, `monthlyAmount`; после Фазы 2 — `λ_pipe` по cure-модели (`pipelineExpected = null` без 3 мес. `stagehistory`). Приёмка: два числа не смешиваются в одной колонке. Фаза 1b / 2.
**Как сделано (сверено 14.09.2026).** Порог «горячего» — решение владельца А.2: стадия ≥ «В решении» (`sales_in_progress`, order 8), константа `AI_ANALYTICS_HOT_STAGE_CODE` (`apps/kpi-report-sales/src/ai-analytics/constants/ai-overview.const.ts:262`). Режется он **по порядку стадии в памяти** — `hotDealsByStageOrder(deals, minStageOrder)` (`domain/loaders/finance-pipeline.assembler.ts:69`) поверх фабрики финансов с широким порогом `presentation`. Набор `SALES_HOT_THRESHOLDS` остался `['presentation', 'document']` (`sales-finance/constants/sales-finance.const.ts:50`): значения `decision` там нет и не будет — это публичный enum запроса и ответа `hot-clients`, его расширение «разъехало бы числа вкладки „Финансы“ и share-link». Строку «в `SALES_HOT_THRESHOLDS` добавляется `decision`» из таблицы §10 читать как устаревшую.
...
**FR-111 Флаг фичи во фронте.** `EAccessFeature.AI_TAB / AI_VIEW_ALL / AI_CONFIGURE` по образцу `PLANS_*`; `AI_CONFIGURE` — `cup|op`; listener на `appActions.setAppData` → `settings/get` → `appActions.setFeatures`. Приёмка: без портального флага вкладки нет; `/share` — скрыта по `selectIsPublic`.
**Как сделано (сверено 14.09.2026 в `front/apps/kpi-sales`; требование уточнено по факту).** Флаг **двухуровневый и оба уровня — поля `features`**: константа приложения `APP_FEATURES.aiAnalytics = true` (`modules/app/consts/features.ts:22` — **не `false`**) и портальный `aiAnalyticsPortalEnabled = false` по умолчанию (:24), который проставляет listener. Предикат `isAiEnabled = features.aiAnalytics && features.aiAnalyticsPortalEnabled` (`modules/shared/access/access.rules.ts:4-5`); `AI_TAB` и `AI_VIEW_ALL` = `isAiEnabled(ctx) && (ctx.isSuperUser || ctx.headOf !== null)`, `AI_CONFIGURE` = `isAiEnabled(ctx) && (isSuperUser || headOf === 'op' || headOf === 'cup')`.
**Чего нет и не требуется:** ветки `isSelf` в `AI_TAB` — отменена решением владельца А.4 (витрина только руководителям, `ctx.isSelf` объявлен, но правилами не используется, `access.types.ts:95-96`); **`isDevOverride` и `NEXT_PUBLIC_AI_ANALYTICS_FORCE` — 0 вхождений** во всём `apps/kpi-sales`, dev-override не реализован и из требования снят (вопрос 12.10 плана закрыт первой трактовкой: портальный флаг не зависит от окружения).
**Уточнения:** путь listener'а — `modules/feature/ai-flags/model/listeners/ai-flags.listener.ts` (а не `model/ai-flags.listener.ts`), слушает `setAppData` **и** `setViewAsUser`, при `features.aiAnalytics === false` не делает даже запроса, пишет `setFeatures({ aiAnalyticsPortalEnabled: settings?.enabled ?? false })`. Портальную настройку `selfViewEnabled` фронт пока **не читает** — открытая находка M19 аудита. Статус: код Фазы 1b написан, но **не закоммичен** (см. §8).
...
| Фаза | Объём | Критерий готовности | Оценка | Статус 06.09.2026 | Статус 14.09.2026 (сверено по коду) |
|---|---|---|---|---|---|
| 0. Аудит данных | `libs/sales-ai-analytics/src/audit`, CLI, админ-ручки, крон, признак, самоописание, страница в админке | отчёт по референсному порталу в `ai/tasks/ai-analytics-audit-<домен>-<дата>.md`; решение по порогам 4.11 и `minDurationSec`; покрытие ≥ 95 %, `other + irrelevant` ≤ 10 % | 3 дня | бэк сделан; фронт в работе; приёмка владельцем и А.1 открыты | **бэк и админ-ручки готовы, отчёта нет**: в `ai/tasks` нет файла `ai-analytics-audit-<домен>-<дата>.md` (есть только аудит выполнения Фазы 2 и аудит bitrix-клиента), решение по порогам 4.11 и пилотный портал 60 с — за владельцем (находка M13) |
| 1a. Push-контур | версии, раскол `call-report-analytics`, lib (пульс/повестка/дайджест/XmR/Уилсон), feature-модуль (settings/pulse/agenda/feedback/cache/push), алерты, 6 ключей | повестка пн 08:30 с 3 звонками; алерт ≤ 10 мин; дайджест с `alternatives`; версии в 100 % разборов; Swagger event-sales без новых роутов; `feedback/list` отдаёт несогласия; менеджер не видит чужих | 8 дней | код сделан и покрыт тестами; **не закоммичено, не задеплоено, на портале не проверено** | **сделано и закоммичено** (`7d68f40`, `28d750f`, далее прод-фиксы `bad5f60…e6f1fde`, `2e0206f`): версии разбора, пульс/повестка/дайджест/XmR/Уилсон в lib, 7 роутов `ai-analytics` в kpi-report-sales, алерты и ключи настроек; на портале проверено прод-фиксами 08–09.09 |
| 1b. Вкладка и таблица | KPI-слой с помесячным кэшем и per-type батчем; `shrink`, `manager-type-matrix`, `attention`, `explanation-template`; `overview/attention/by-type/settings/save`; фронт (флаги, listener, entity, виджеты) | 0 расхождений с существующими ручками; при n < 8 ни одного числа; повторный клик — `processing`; overview ≤ 60 с; «длинный» режим воспроизводит постановку; вкладка по флагу; менеджер видит только себя | 14 дней (9 бэк + 5 фронт) | не начато; пререквизит — `pnpm generate` после деплоя | **бэк сделан, фронт написан, но не закоммичен.** Бэк: 11 роутов в двух контроллерах (`overview`, `attention`, `by-type`, `settings/save`, `settings/get`, `pulse`, `agenda`, `feedback`, `feedback/list`, `cache/reset`, `push`), KPI-слой с помесячным кэшем, `shrink`/`manager-type-matrix`/`attention`/`explanation-template` в lib. Фронт: слайс `apps/kpi-sales/modules/entities/ai-analytics` в git **не числится** (`git ls-files` — 0 файлов; HEAD `front` от 06.09, 226 изменённых/новых файлов в рабочей копии) — блокер B4 аудита. Формулировка «не начато» устарела |
| 2. Нормы, план дня, резюме, снапшоты | `findByDomainTypeKeys` + индексы, `structuredCompletionWithUsage`, `stagehistory`; ночной конвейер и снапшоты; реестр; LOO-усадка; `plan/daily`; `brief`; форма настроек; test-retest | синтетика ≥ 85 % в 90 %-интервале; ≤ 5 % ложных утечек; ≥ 95 % буллетов проходят факт-чек; план ≤ потолка; `pipelineExpected = null` без стадий | 22 дня | не начато | **в работе, 16 потоков из 20 приняты.** Сделано: реестр параметров (195 кодов), снапшоты и стор, вся математика (экспозиция, κ Клейнмана, LOO-нормы, эпизоды, cure-прогноз, qav/рычаги, готовность и AI-резюме), ключи настроек, ночной конвейер с ритмами и backfill, история стадий, паспорт, неделя/месяц/стиль, три звонка недели, портальная модель, нормы в витрине; домен `calendar` в `libs/bitrix`. **Не сделано:** потоки 17 `plan/daily`, 18 `brief`, 19 сборка (конвейер не подключён к `AiAnalyticsModule`, `PortalModelStep`/`ForecastStep` вне порядка шагов — блокер B2), 20 фронт. Волна A доделок 14.09 закрыла B1, B3, M1–M7 и часть minor; детали — `ai/tasks/ai-phase2-audit-2026-09-14.md` и §11 плана Фазы 2 |
| 3. Досье, тренды, реконсиляция, админ | EWMA/перестановки/CUSUM, ramp, plan-fact в `libs/call-lib`, `dossier`, админ-модуль (recompute, backfill, etl-status, golden-set, retention), сегменты → `timecodes` | < 1 ложной тревоги/год/менеджер; ступенька ×1,3 ≤ 3 окна; линия рвётся на `comparableFrom`; golden set ICC ≥ 0,7 | 20 дней | не начато | не начато (14.09.2026) |
| 4. Прогноз и эмпирический β | Клейнман-κ, `F(d)` KM, NegBin, логнормальный чек, иерархический β на пуле, бэктест, readiness | coverage накрывает 0,8; MASE < 1 vs naive; `beta_gate` два месяца подряд; E1 на референсном портале — через 8–10 мес. после старта Фазы 2 | 15 дней + 9–12 мес. shadow | не начато | не начато (14.09.2026) |
