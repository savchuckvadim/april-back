...
export class InfogroupPrismaRepository implements InfogroupRepository {
    ...
    async addInfoblocks(
        ...
        try {
            await this.prisma.infoblock.updateMany({
                where: {
                    id: {
                ...
                data: {
                    group_id: BigInt(infogroupId),
                },
            });
            return this.findById(Number(infogroupId));
        } catch (error) {
    ...
    async setInfoblocks(
        ...
        try {
            // Удаляем все инфоблоки из группы
            await this.prisma.infoblock.updateMany({
                where: {
                    group_id: BigInt(infogroupId),
                },
            ...
            // Добавляем новые инфоблоки в группу
