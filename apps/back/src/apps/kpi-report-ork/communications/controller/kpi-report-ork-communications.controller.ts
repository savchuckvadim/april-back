...
export class CommunicationsController {
    ...
    @ApiResponse({
        status: 200,
        description: 'Report',
        type: GetReportCommunicationsResponseDto,
    })
    @Post('get')
    async get(
        @Body() dto: GetReportCommunicationsRequestDto,
    ): Promise<GetReportCommunicationsResponseDto> {
        if (!dto.domain) {
            throw new BadRequestException('Domain is required');
        }
        const useCase = new ReportCommunicationsUseCase(this.pbx);
