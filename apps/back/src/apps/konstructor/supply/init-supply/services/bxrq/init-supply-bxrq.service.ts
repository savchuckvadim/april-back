...
import {
    ...
    BxRqBankItemDto,
    BxRqDto,
} from 'src/apps/konstructor/document-generate/dto/bx-rq/bx-rq.dto';
import {
    BX_ADDRESS_TYPE,
    RQ_ITEM_CODE,
...
export class InitSupplyBxrqService {
    ...
    public getInnValue(rq: BxRqDto): string {
        let innValue = '';
        const innField = rq.fields.find(
            field => field.code === RQ_ITEM_CODE.INN,
        );
        if (innField) {
    ...
    public getFullNameValue(rq: BxRqDto, clientType: ClientTypeEnum): string {
        ...
        const fullNameField =
            clientType === ClientTypeEnum.FIZ
                ? rq.fields.find(
                      field => field.code === RQ_ITEM_CODE.PERSON_NAME,
                  )
