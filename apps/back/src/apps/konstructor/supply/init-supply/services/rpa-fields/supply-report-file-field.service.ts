...
export class InitSupplyRpaSupplyReportFileFieldService {
    ...
    public async get(dto: InitSupplyDto, PortalModel: PortalModel) {
        const currentSupplyReportBxFieldData =
            await this.initSupplyFileService.downloadFileAndConvertToBase64(
                dto.file,
            );
        const rpaCurrentSupplyReportField =
            PortalModel.getRpaFieldBitrixIdByCode('supply', 'current_supply');
