...
export class BitrixTokenService {
    ...
    // BitrixToken methods
    async storeOrUpdateToken(
        dto: CreateBitrixTokenDto,
    ): Promise<{ token: BitrixTokenEntity; message: string }> {
        try {
            ...
            if (!app) {
                throw new NotFoundException(
                    `App with code ${dto.code} not found`,
                );
            }
    ...
    async storeOrUpdateAppToken(
        ...
        try {
            ...
            const token =
                await this.repository.storeOrUpdateTokensWithoutSecrets(
                    appId,
            ...
            if (!token) {
                throw new BadRequestException(
                    'Failed to create or update token',
                );
            }
    ...
    async storeOrUpdateAppSecret(
        appId: bigint,
        dto: SetBitrixSecretDto,
    ): Promise<{ token: BitrixTokenEntity; message: string }> {
        try {
            // Encrypt token data
            ...
            console.log(
                'storeOrUpdateAppSecret encryptedClientId',
                encryptedClientId,
            );
            console.log(
                'storeOrUpdateAppSecret encryptedClientSecret',
