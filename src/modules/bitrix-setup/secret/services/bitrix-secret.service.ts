...
export class BitrixSecretService {
    constructor(private readonly repository: BitrixSecretRepository) {}

    // Encryption/Decryption methods (similar to Laravel's Crypt)

    // BitrixSecret methods
    async storeOrUpdateSecret(
        dto: CreateBitrixSecretDto,
    ): Promise<{ app: BitrixSecretEntity; message: string }> {
        try {
            ...
            if (!app) {
                throw new BadRequestException(
                    'Failed to create or update app secret',
                );
            }
    ...
    async getSecretByCode(dto: GetBitrixSecretDto): Promise<{
        ...
        if (!app) {
            throw new NotFoundException(
                `App secret with code ${dto.code} not found`,
            );
        }
