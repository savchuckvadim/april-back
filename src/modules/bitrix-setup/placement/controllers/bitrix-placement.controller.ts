...
export class BitrixPlacementController {
    ...
    async store(@Body() dto: CreateBitrixPlacementDto) {
        try {
            const result =
                await this.bitrixPlacementService.storePlacements(dto);
    ...
    async getByApp(@Param('appId') appId: string) {
        try {
            const placements =
                await this.bitrixPlacementService.getPlacementsByAppId(
                    BigInt(appId),
                );
    ...
    async deleteByApp(@Param('appId') appId: string) {
        try {
            const result =
                await this.bitrixPlacementService.deletePlacementsByAppId(
                    BigInt(appId),
                );
    ...
    async delete(@Param('id') id: string) {
        try {
            const result = await this.bitrixPlacementService.deletePlacement(
                BigInt(id),
            );
