...
export const createBitrixAppEntityFromPrisma = (
    ...
    >,
    bitrix_tokens?: NonNullable<Awaited<
        ReturnType<PrismaService['bitrix_tokens']['findFirst']>
    > | null>,
    bitrix_app_placements?: NonNullable<Awaited<
        ReturnType<PrismaService['bitrix_app_placements']['findFirst']>
    > | null>,
    bitrix_settings?: NonNullable<Awaited<
        ReturnType<PrismaService['bitrix_settings']['findUnique']>
    > | null>,
    portal?: NonNullable<Awaited<
        ReturnType<PrismaService['portal']['findUnique']>
    > | null>,
): BitrixAppEntity => {
    ...
    entity.bitrix_tokens = bitrix_tokens
        ? createBitrixTokenEntityFromPrisma(bitrix_tokens)
        : undefined;
...
export const getRelations = async (
    ...
    bitrix_settings?:
        | NonNullable<Awaited<
              ReturnType<PrismaService['bitrix_settings']['findFirst']>
          > | null>
        | undefined;
    portal?:
        | NonNullable<Awaited<
              ReturnType<PrismaService['portal']['findFirst']>
          > | null>
        | undefined;
}> => {
    const bitrix_tokens =
        (await prisma.bitrix_tokens.findFirst({
            where: { bitrix_app_id: appId },
        })) || undefined;
    const bitrix_app_placements =
        (await prisma.bitrix_app_placements.findFirst({
            where: { bitrix_app_id: appId },
        })) || undefined;
    const bitrix_settings =
        (await prisma.bitrix_settings.findFirst({
            where: { settingable_id: appId },
        })) || undefined;
    const portal =
        (await prisma.portal.findFirst({
            where: { id: portalId },
        })) || undefined;
