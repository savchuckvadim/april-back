...
import { bitrixfield_items, bitrixfields } from 'generated/prisma';
...
export class BitrixFieldService {
    ...
    async findMany(): Promise<BitrixFieldResponseDto[]> {
        ...
        return fields.map(f => this.mapToResponseDto(f));
    }
    ...
    async findByEntity(
        ...
        const fields = await this.repository.findByEntity(
            prismaEntityType,
            entityId,
        );
        if (!fields) {
    ...
    private mapToResponseDto(field: bitrixfields): BitrixFieldResponseDto {
        const fieldWithItems = field as bitrixfields & {
        ...
        return {
            ...
            code: field.code,
            items: (fieldWithItems.bitrixfield_items ?? []).map(item => ({
                id: Number(item.id),
