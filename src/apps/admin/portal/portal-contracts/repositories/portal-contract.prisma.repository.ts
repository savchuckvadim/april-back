...
@Injectable()
export class PortalContractPrismaRepository
    implements PortalContractRepository
{
        ...
        const result = await this.prisma.portal_contracts.create({
            data: {
                ...
                bitrixfield_item_id: BigInt(
                    portalContract.bitrixfield_item_id!,
                ),
    ...
    async update(
        ...
        if (portalContract.contract_id !== undefined)
            updateData.contract_id = BigInt(portalContract.contract_id);
        if (portalContract.portal_measure_id !== undefined)
            updateData.portal_measure_id = BigInt(
                portalContract.portal_measure_id,
            );
        if (portalContract.bitrixfield_item_id !== undefined)
            updateData.bitrixfield_item_id = BigInt(
                portalContract.bitrixfield_item_id,
            );
        if (portalContract.title !== undefined)
            updateData.title = portalContract.title;
        if (portalContract.template !== undefined)
            updateData.template = portalContract.template;
        if (portalContract.order !== undefined)
            updateData.order = portalContract.order;
        if (portalContract.productName !== undefined)
            updateData.productName = portalContract.productName;
        if (portalContract.description !== undefined)
            updateData.description = portalContract.description;
