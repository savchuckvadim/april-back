...
export class RpaCommentHelper {
    ...
    public static getList(list: string[], withMarginTop: boolean = false): string {
        return `<ul style="margin-top: ${withMarginTop ? '10px' : '0px'}">${list.map(item => item ? `<li>${item}</li>` : '').join('')}</ul>`
    }

    public static getListWithBold(list: { head: string, value: string }[], withMarginTop: boolean = false): string {
        return `<ul style="margin-top: ${withMarginTop ? '10px' : '0px'}">${list.map(item => `<li>${this.getBoldString(item.head)} ${item.value}</li>`).join('')}</ul>`
    }

    public static getHeaderWithIcon(iconType: RpaIconEnum, text: string): string {
        ...
        return this.getBoldString(`<div style="${style} display: flex; align-items: center; gap: 5px;">${icon} ${text}</div>`);
    }
