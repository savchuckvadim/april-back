...
export class InfoblockGeneralService {
    getInfoblockGeneralComment(
        contractSpecification: ContractSpecificationDto,
    ): string {
        const infoblockGeneral = contractSpecification.items.find(
            item => item.code === ContractSpecificationCodeEnum.IBLOCKS,
        );
        const infoblockGeneralValue = infoblockGeneral?.value as string;
        const ersData = contractSpecification.items.find(
            item => item.code === ContractSpecificationCodeEnum.IERS,
        );
        ...
        if (infoblockGeneralValue) {
            const iblocks = infoblockGeneralValue
                .split('\n')
                .map(item => item.trim());
        ...
        const freeBlocksComment = this.getFreeBlocksComment(
    ...
    private getFreeBlocksComment(
        contractSpecification: ContractSpecificationDto,
    ): string {
        const freeBlocks = contractSpecification.items.find(
            item => item.code === ContractSpecificationCodeEnum.IFREE,
        );
    ...
    private getErPacketsComment(
        contractSpecification: ContractSpecificationDto,
    ): string {
        const packetErsData = contractSpecification.items.find(
            item => item.code === ContractSpecificationCodeEnum.IERS_PACKETS,
        );
        const ersInPacket = contractSpecification.items.find(
            item => item.code === ContractSpecificationCodeEnum.IERS_IN_PACKETS,
        );
