...
export class TotalSumCommentService {
    ...
    getTotalSumComment(dto: InitSupplyDto): string {
        ...
        if (total && total.price) {
            ...
            const totalSumString = RpaCommentHelper.getBoldString(`Всего за ${quantity} мес.: ${totalSum} руб.`)
            const totalMonthSumString = RpaCommentHelper.getBoldString(`В месяц: ${totalMonthSum} руб.`)


            const paragrafName = RpaCommentHelper.getHeaderWithIcon(
                RpaIconEnum.TOTAL_SUM, 'Сумма:')
            const list = [
                totalSumString,
                totalMonthSumString
            ]
