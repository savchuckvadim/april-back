...
export class InitSupplyRpaRqFieldsService {
    ...
    public async get(dto: InitSupplyDto, PortalModel: PortalModel) {
        ...
        const bxrqValues = this.getBxrqValues(dto.bxrq, dto.clientType.code, PortalModel)
    ...
    private getBxrqValues(
        ...
        const {
            inn,
            address
        } = this.initSupplyBxrqService.getBxrqValues(bxrq, clientType)

        const innRpaFieldBitrixId = PortalModel.getRpaFieldBitrixIdByCode('supply', 'company_rq_inn')
        const addressRpaFieldBitrixId = PortalModel.getRpaFieldBitrixIdByCode('supply', 'service_address')
