    ...
    async initSupply(dto: InitSupplyDto): Promise<any> {
        ...
        **/
        ...
        if (dto.rpa_id && rpaTypeId) {
            rpaResponse = await bitrix.rpaItem.update({
                id: dto.rpa_id,
        ...
        const timelineComment = await this.initSupplyTimelineCommentService.getTimelineComment(dto, PortalModel)
        if (rpaId && dto.userId) {

            rpaTimelineResponse = await bitrix.api.call(
                'rpa.timeline.add',
                {
                    typeId: rpaTypeId,
            ...
            dto.dealId && await bitrix.timeline.addTimelineComment({
                ENTITY_ID: Number(dto.dealId),
                ENTITY_TYPE: EBXEntity.DEAL,
                COMMENT: this.getCommentEntityMessage(dto.domain, rpaTypeId, rpaId),
