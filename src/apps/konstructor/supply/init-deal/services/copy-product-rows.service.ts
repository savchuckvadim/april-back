import { BitrixService, IBXDealProductRowGet, IBXProductRowRow } from "@/modules/bitrix";
...
export class CopyProductRowsService {
    ...
    async copyProductFromSmartToDeal(serviceSmartId: number, portalSmart: IPSmart) {

        const smartEntityTypeId = portalSmart.entityTypeId
        const smartCrmId = portalSmart.crm
        const smartCrmTypeId = smartCrmId.endsWith('_') ? smartCrmId.slice(0, -1) : smartCrmId;
        ...
        for (const row of rows) {
            ...
            for (const key in row) {
                if (key !== 'id' && key !== 'ownerId' && key !== 'ownerTypeId') {
                    newRow[key] = row[key]
                }
            }
            newRows.push(newRow)
