...
export class InitDealUseCase {
    ...
    async execute(dto: InitDealDto) {
        ...
        const targetCategoryDeal = portalDeal.categories.find(category => category.code === 'service_base')
        ...
        const offerSmartInRpaId = PortalModel.getRpaFieldBitrixIdByCode('supply', 'service_offer_smart')
    ...
    private async getDealValuesFromOfferSmart(
        ...
        for (const key in offerSmartItem) {
            const portalSmartField = portalSmart.bitrixfields.find(field => field?.bitrixCamelId === key)
    ...
    private async getDealFieldValuesFromRpa(
        ...
        for (const key in rpa) {
            const portalRpaField = portalRpa.bitrixfields.find(field =>
    ...
    private async prepareFieldValue(rawValue: any, bitrix: BitrixService, fldCode: string): Promise<any> {
        let value = rawValue
        ...
        } else if (Array.isArray(rawValue) && rawValue.every(item => Array.isArray(item))) {
            value = rawValue.flat()
    ...
    private getCompanyIdFromRpa(rpa: IBxRpaItem, portalModel: PortalModel): number | null {
        const rpaField = portalModel.getRpaFieldBitrixIdByCode('supply', 'rpa_crm_company')
        if (rpaField) {
    ...
    private getDealIdFromRpa(rpa: IBxRpaItem, portalModel: PortalModel): number | null {
        const rpaField = portalModel.getRpaFieldBitrixIdByCode('supply', 'rpa_crm_base_deal')
        if (rpaField) {
            return Number(rpa[rpaField])
        }
        return null
    ...
    private getRpaFieldBitrixId(rpaTypeId: number, fieldBitrixId: number | string): string {
