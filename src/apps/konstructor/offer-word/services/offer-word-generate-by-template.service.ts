...
export class OfferWordGenerateByTemplateService {
    ...
    async generateOfferWord(dto: OfferWordByTemplateGenerateDto) {
        ...
        const complectCodes = dto.complect.map(group =>
        ...
        const regions = [
            ...dto.regions.inComplect,
            ...dto.regions.favorite,
            ...dto.regions.noWidth,
        ];
        const provider = await this.providerService.findById(
            Number(dto.providerId),
        );
        const link = await this.generateByTemplate(
            template,
            dto.domain,
            dto.userId.toString(),
        );
    ...
    private async generateByTemplate(
