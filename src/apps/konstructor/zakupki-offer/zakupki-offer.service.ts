...
export class ZakupkiOfferCreateService {
    ...
    async createZakupkiOffer(dto: ZakupkiOfferCreateDto) {
        ...
        const otherProvider1Data = await this.getProviderOtherData(
            dto.otherProviders,
            'provider1',
        );
        const otherProvider2Data = await this.getProviderOtherData(
            dto.otherProviders,
            'provider2',
        );
    ...
    private async getProviderOtherData(
        otherProvidersInitData: OtherProvidersDto,
        type: 'provider1' | 'provider2',
    ) {
        const provider = otherProvidersInitData[type];
        let enumchick = Provider1FieldCode as
            | typeof Provider1FieldCode
            | typeof Provider2FieldCode;
        if (type === 'provider2') {
            enumchick = Provider2FieldCode;
        }
        const providerCompanyName = provider.find(
            provider => provider.code === enumchick.SHORTNAME,
        );
        const providerAddress = provider.find(
            provider => provider.code === enumchick.ADDRESS,
        );
        const providerPhone = provider.find(
            provider => provider.code === enumchick.PHONE,
        );
        const providerEmail = provider.find(
            provider => provider.code === enumchick.EMAIL,
        );
        const providerInn = provider.find(
            provider => provider.code === enumchick.INN,
        );
        const providerPosition = provider.find(
            provider => provider.code === enumchick.POSITION,
        );
        const providerLetterText =
            provider
                .find(provider => provider.code === enumchick.LETTER_TEXT)
                ?.value?.toString() || ('' as string);
        const providerDirector = provider.find(
            provider => provider.code === enumchick.DIRECTOR,
        );
    ...
    private getSumsProviders(
        ...
        const searchedCefficient1: number = Number(
            providers.provider1.find(
                provider =>
                    provider.code === Provider1FieldCode.PRICE_COEFFICIENT,
            )?.value,
        );
        ...
        const searchedCoefficient2: number = Number(
            providers.provider2.find(
                provider =>
                    provider.code === Provider2FieldCode.PRICE_COEFFICIENT,
            )?.value,
        );
    ...
    async setInBitrix(
        ...
        const commentMessage = this.getCommentMessage(
            link,
