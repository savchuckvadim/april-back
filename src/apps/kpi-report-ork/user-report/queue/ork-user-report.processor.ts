...
import { OrkUserReportGetRequestDto, OrkUserReportJobDataDto } from '../dto/ork-user-report.dto';
import { RedisService } from '@/core/redis/redis.service';
export enum USER_REPORT_EVENT {
    PROGRESS = 'ork-user-report:progress',
    DONE = 'ork-user-report:done',
}
@Processor(QueueNames.ORK_KPI_REPORT)
export class OrkUserReportQueueProcessor {
    ...
    @Process(JobNames.ORK_USER_REPORT_GENERATE)
    async handle(job: Job<OrkUserReportJobDataDto>) {
        ...
        try {
            ...
            let batchCount = 0;
            for await (const batch of this.orkUserReportService.getReport(job.data)) {
                ...
                this.ws.sendToClient(socketId, {
                    event: USER_REPORT_EVENT.PROGRESS,
                    userId,
                    data: batch,
                });
                batchCount++;
