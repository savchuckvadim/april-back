...
export class EventSalesFlowUseCase {
    ...
    async getFlow(domain: string): Promise<{ result: boolean, error: string, data: any }> {
    ...
    protected async getNEWBatchDealFlow(
        ...
        const isUnplanned = isPresentationDone && currentReportEventType !== 'presentation';
        ...
        if (!isPostSale) {
            if (!currentBtxDeals) {
                const setNewDealData = {
                    ...
                    CATEGORY_ID: portalDealData.categories.find(c => c.code === 'sales_base')?.bitrixId.toString(),
                    ASSIGNED_BY_ID: planResponsibleId.toString()
                };

                currentDealId = await this.bitrixDealService.setDeal(hook, setNewDealData);

                if (currentDealId && !currentBaseDeal) {
                    const newBaseDeal = await this.bitrixDealService.getDeal(hook, { id: Number(currentDealId) });
        ...
        for (const category of portalDealData.categories) {
            switch (category.code) {
                case 'sales_base': {
                    const currentStageOrder = this.bitrixDealService.getEventOrderFromCurrentBaseDeal(currentBaseDeal, category);
                    const targetStageBtxId = this.bitrixDealService.getSaleBaseTargetStage(
                        category,
                        currentStageOrder,
                    ...
                    if (currentDealId) {
                        const batchCommand = this.bitrixDealBatchFlowService.getBatchCommand(fieldsData, 'update', Number(currentDealId));
                        const key = `update_${category.code}_${currentDealId}`;
                        resultBatchCommands[key] = batchCommand;
                    } else {
                        const batchCommand = this.bitrixDealBatchFlowService.getBatchCommand(fieldsData, 'add', null);
                ...
                case 'sales_xo': {
                    if (!isNoCall) {
                        const targetStageBtxId = this.bitrixDealService.getXOTargetStage(
                            category,
                            currentReportEventType,
                            isExpired,
                            isResult,
                        ...
                        if (xoDealId) {
                            const batchCommand = this.bitrixDealBatchFlowService.getBatchCommand(fieldsData, 'update', Number(xoDealId));
                    ...
                    if (!isNoCall) {
                        if (currentReportEventType === 'presentation') {
                            if (reportPresDealId) {
                                ...
                                const targetStageBtxId = this.bitrixDealService.getTargetStagePresentation(
                                    category,
                                    currentReportStatus,
                                    isResult
                                );
                                ...
                                const batchCommand = this.bitrixDealBatchFlowService.getBatchCommand(fieldsData, 'update', Number(reportPresDealId));
                                const key = `update_${category.code}_${reportPresDealId}`;
                                resultBatchCommands[key] = batchCommand;

                                const entityCommand = this.getEntityBatchFlowCommandFromIdForNewDeal(
                                    true,
                                    reportPresDealId,
                                    'presentation',
                                    Number(currentBaseDeal?.ID),
                            ...
                            if (currentTMCDeal) {
                                fieldsData['UF_CRM_TO_BASE_TMC'] = currentTMCDeal.ID;
                            }
                        ...
                        if (isUnplanned) {
                            ...
                            const entityCommand2 = this.getEntityBatchFlowCommandFromIdForNewDeal(
                                true,
                                unplannedPresDeal,
                                'presentation',
                ...
                case 'tmc_base': {
                    if (!isNeedReturnToTmc && !isNoCall) {
                        if (currentTMCDeal && currentPlanEventType === 'presentation') {
                            ...
                            const fieldsData = {
                                ...
                                UF_CRM_LAST_PRES_DONE_RESPONSIBLE: planResponsibleId,
                                UF_CRM_MANAGER_OP: planResponsibleId
                            };

                            const batchCommand = this.bitrixDealBatchFlowService.getBatchCommand(fieldsData, 'update', Number(currentTMCDeal.ID));
        ...
        await this.bitrixBatchService.sendGeneralBatchRequest(resultBatchCommands);
