...
export type EntityFlowResponse = IBXCompany | IBXLead | IBXDeal | IBXContact;
export class BitrixEntityFlowService {
    private readonly bitrix: BitrixService;
    private readonly logger = new Logger(BitrixEntityFlowService.name);
    constructor(bitrix: BitrixService) {
        this.bitrix = bitrix;
    }
    public async getWorkstatusFieldItemValue(
        entityType: 'company' | 'lead' | 'deal' | 'contact',
        entityId: number,
        workStatus: string,
        planEventType: string,
    ): Promise<EntityFlowResponse | null> {
        const entity = await this.bitrix[entityType].get(entityId);
        const entityData = entity.result;
        this.logger.log('entityData', entityData);
        this.logger.log('workStatus', workStatus);
        this.logger.log('planEventType', planEventType);
        return entityData;
    }
}
