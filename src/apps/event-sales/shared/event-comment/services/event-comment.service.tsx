...
export class EventCommentService {
    constructor() {}
    getComment(data: IGetComment): string {
        const {
            deadline,
            comment,
            currentPlanEventTypeName,
        ...
        if (isPlanned && !isNoCall) {
            if (!isExpired) {
                ...
                if (
                    this.removeEmojisIntl(currentPlanEventTypeName) ===
                    'Презентация'
                ) {
    ...
    protected removeEmojisIntl(string: string): string {
        ...
        // - Flags (1F1E6-1F1FF)
        // - Skin tone modifiers (1F3FB-1F3FF), VS16 (FE0F), ZWJ (200D) — separate .replace (not in [...])
        // - Dingbats (2700-27BF)
        // - Enclosed characters (24C2-1F251)

        return string
            .replace(
                /[\u{1F000}-\u{1FAFF}\u{2600}-\u{26FF}\u{1F1E6}-\u{1F1FF}\u{2700}-\u{27BF}\u{24C2}-\u{1F251}]/gu,
                '',
            )
            .replace(/\u{1F3FB}|\u{1F3FC}|\u{1F3FD}|\u{1F3FE}|\u{1F3FF}/gu, '')
            .replace(/\u{FE0F}/gu, '')
            .replace(/\u{200D}/gu, '')
            .trim();
    }
